  ...
  <div class="container-cozi py-12 sm:py-20">
    ...
    <section class="mt-12 grid gap-4 md:grid-cols-2" aria-label="News posts">
      <NuxtLink v-for="post in posts" :key="post.path" class="group rounded-3xl border border-cozi-line bg-white/[0.045] p-6 no-underline shadow-cozi transition hover:-translate-y-1 hover:border-cozi-amber sm:p-8" :to="post.path">
        ...
        <time class="text-xs font-black uppercase tracking-[0.12em] text-cozi-pink" :datetime="post.date">{{ formatDate(post.date) }}</time>
        <h2 class="mt-2 mb-0 font-display text-3xl font-black text-cozi-cream">{{ post.title }}</h2>
        <p class="mt-3 mb-0 text-cozi-muted">{{ post.description }}</p>
        <span class="mt-6 inline-flex items-center gap-2 text-xs font-black uppercase tracking-wider text-cozi-amber">Read update <Icon name="lucide:arrow-right" aria-hidden="true" /></span>
      </NuxtLink>
    </section>
  </div>
</template>
